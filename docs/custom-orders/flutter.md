...
## 7) UI قسم الطلب السريع (الهوم + الهيدر)
...
1. اقرأ `quick_order.is_enabled` عند فتح الهوم (أو من كاش الإعدادات).
2. إن `false` → لا تعرض زر «طلب سريع / Urgent» في الهيدر ولا البانر.
3. إن `true` → اعرض الزر؛ النقر يُظهر/يخفي القسم أو يفتح شاشة الإنشاء حسب UX المنتج (الموصى: الزر يفتح الشاشة، والقسم يظهر دائماً تحت الهيدر عندما مفعّل).
