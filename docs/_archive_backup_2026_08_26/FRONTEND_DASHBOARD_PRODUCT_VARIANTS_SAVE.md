...
هذا يمنع حالة "منتج بلا متغيّر".

إذا ما أُرسل `shop_variants` (أو المتغيّر بقي بلا ربط فرع)، الـ Backend يربط تلقائيًا
بـ **فرع البائع الافتراضي** (`shops.is_default = true`، وإن ما وُجد → أول فرع نشط لنفس البائع).
فالمنتج يصير قابل للشراء على الموقع بدون اختيار فرع يدويًا.

إذا أرسلت `shop_variants` مع `shop_id` → تُحفظ كما هي ولا يُستبدل الربط.

**ملاحظة للداشبورد:** تحذير «المنتج غير مرتبط بأي فرع» لازم يختفي بعد الحفظ إذا عند البائع فرع `is_default`.
تأكد أن لبائع المنصة (أو البائع المختار) فرع واحد على الأقل معلّم كافتراضي.
...
| لا تُرسل `shop_variants` نهائياً | الروابط الحالية **تبقى كما هي**؛ وأي متغيّر بلا ربط يُربَط بفرع البائع الافتراضي |
| `shop_variants` = مصفوفة فاضية | تُحذف الروابط ثم يُعاد الربط تلقائياً بفرع البائع الافتراضي (`is_default`) |
...
بالفروع — هذا يستبدل الروابط الحالية بفرع البائع الافتراضي فقط (يفقد ربط الفروع الأخرى).
...
إذا `variants` فاضية أو `shops` فاضية بعد الحفظ → غالباً ما في فرع افتراضي (`is_default`) للبائع.
تأكد من وجود فرع افتراضي نشط؛ وإلا اعرض تنبيه (مثلاً badge "غير مرتبط بفرع").
