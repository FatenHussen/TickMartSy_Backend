...
**الويب (قرار مقصود):** كل الأقسام تُعرض كـ **سلايدر أفقي** (تمرير يمين/يسار). لا تُستخدم الشبكات الثابتة — لأنها تحمّل كل العناصر دفعة واحدة.

`variant` يحدّد **شكل الكارد** فقط (وليس نوع التخطيط):
...
> شكل الكارد يُشتق أيضًا من `display_type_id` إن رغبت بتوحيد الأشكال؛ وإلا اعتمد `variant`.
