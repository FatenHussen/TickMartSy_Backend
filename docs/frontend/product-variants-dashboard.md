...
|---|----------------|-----|-----------|
| — | **الصفات** (لون · مقاس · تصميم) | `attributes_values_ids` | **اختياري لكل dropdown** — اختاري 1 أو 2 أو 3 — مو إلزام الكل ([§4 UX](./DASHBOARD_PRODUCT_CREATE_UX.md)) |
...
### كل الحقول — **مو required** (بما فيها الصفات)

> **لا تضع `*` أو `required` على أي حقل** — لا في الكارد ولا في dropdowns الإضافة.
> **لا تُجبري** المستخدم يختار كل صفات الفئة — اختيار جزئي مسموح ([DASHBOARD_PRODUCT_CREATE_UX.md](./DASHBOARD_PRODUCT_CREATE_UX.md)).
> النص «املأ كل الحقول» / «اختر الثلاثة» **خطأ**.
...
- **لا كارد افتراضي** عند فتح إنشاء المنتج — القائمة فاضية إلى أن يضغط الأدمن «إضافة».
