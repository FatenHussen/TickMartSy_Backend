# ويب — كل التعديلات (نسخة نهائية — 5 أيلول 2026)
...
- [ ] كمية: `shop_variants[].quantity` مع `null`
...
## 11) الضمان + الكمية (5 أيلول 2026)

> الدليل الكامل للإرسال: [`WEB_LATEST.md`](./WEB_LATEST.md)

### الضمان

`GET /api/user/products/{id}` يرجّع:

```json
"warranty": { "id": 1, "name": "إرجاع مجاني", "description": "…" }
```

`name` و `description` = **string** حسب `Accept-Language`. `warranty` ممكن `null`.

Fallback للمنتجات القديمة: `warranty_period` (أشهر) إذا ما في `warranty`.

لا تستدعوا Admin warranties من الموقع.

### الكمية

المخزون = `shop_variants[].quantity` (`int | null`). كمية المنتج قد تكون `null` — لا تعتمدوا عليها.
...
الأدمن ما عاد يولّد متغيّر فاضي. الباك يبقى يرجّع `shop_variants[0]` fallback حتى ما تكسر الصفحة.
